...
rates_table = Table(
    [
        # GWTC-4 paper (https://arxiv.org/pdf/2508.18083) Table 2 fullpop-4.0, full cell
        {"population": "BNS", "lower": 50.0, "mid": 130.0, "upper": 290.0},
        {"population": "NSBH", "lower": 50.0, "mid": 130.0, "upper": 290.0},
        {"population": "BBH", "lower": 50.0, "mid": 130.0, "upper": 290.0},
    ]
)

# For splitting into BNS, NSBH, and BBH populations.
# Note that p_astro uses 3 Msun but GWTC-4 uses 2.5.
ns_max_mass = 3

# Calculate effective rate density for each sub-population
table = Table.read("fullpop4.h5")
...
for run_name, run_dir in zip(tqdm(run_names), run_dirs):
    path = Path("runs") / run_dir / "fullpop4"
...
o4_data = Table.read("public-alerts.dat", format="ascii")
o4_data["vol(90)"] *= 1e-6
